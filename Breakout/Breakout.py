...
while run:
    ...
    if not smrť:
        if game_over == 0:
            ...
            hudbička = random.randrange(0, 2)
            if hudbička == 0:
                mixer.music.load("Bangers/Medium/7.wav")
                mixer.music.play(-1)
            elif hudbička == 1:
                mixer.music.load("Bangers/Medium/8.wav")
                mixer.music.play(-1)
            ...
            class game_ball():
                ...
                def draw(self):
                    pygame.draw.circle(screen, farba_palky,
                                       (self.rect.x + self.ball_rad, self.rect.y + self.ball_rad),
                                       self.ball_rad)
                    pygame.draw.circle(screen, palka_obtiahnutie,
                                       (self.rect.x + self.ball_rad, self.rect.y + self.ball_rad),
                                       self.ball_rad, 3)
            ...
            while run:
                ...
                if not smrť:
                    if game_over == 0:
                        ...
                        hudbička = random.randrange(0, 3)
                        if hudbička == 0:
                            mixer.music.load("Bangers/Hard/4.wav")
                            mixer.music.play(-1)
                        elif hudbička == 1:
                            mixer.music.load("Bangers/Hard/5.wav")
                            mixer.music.play(-1)
                        elif hudbička == 2:
                            mixer.music.load("Bangers/Hard/6.wav")
                            mixer.music.play(-1)
                        ...
                        while run:
                            ...
                            if not smrť:
                                if game_over == 0:
                                    ...
                                    draw_text('Klikni aby si to skúsil znova', font, text_farba, 490,
                                              výška_obrazovky // 2 + 100)
